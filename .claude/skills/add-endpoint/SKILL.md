---
name: add-endpoint
description: Checklist đầy đủ khi thêm 1 API endpoint mới vào backend PayPilot AI — tra RBAC matrix, gắn @Roles đúng, tenant scoping, cập nhật rbac.md, viết test liên quan. Dùng khi thêm route mới vào controller có sẵn hoặc khi user gõ "/add-endpoint".
---

# Add Endpoint — Checklist thêm API endpoint chuẩn PayPilot AI
...
Trả data thô, để `ResponseInterceptor` toàn cục bọc thành `{ success, data, meta, error }` (type `ApiResponse<T>` từ `@paypilot/shared-types`) — không tự bọc tay trong từng handler.
...
- Chạy `pnpm --filter @paypilot/backend test` trước khi coi là xong.
