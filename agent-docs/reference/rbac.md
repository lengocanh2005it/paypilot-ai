# 🔐 PayPilot AI — RBAC (Role-Based Access Control)
...
Xem doanh thu PayPilot thu được      Không thấy dữ liệu của tenant khác
...
- **Cas Partner** được tạo thủ công bởi đội ngũ PayPilot (không tự đăng ký được qua form thông thường) — vì đây là tài khoản nội bộ đại diện cho đối tác Cas/CASSO.
...
| Xem doanh thu | Doanh thu PayPilot thu từ từng tenant theo gói cước, tổng doanh thu toàn hệ thống |
...
## 💰 Pricing Model — PayPilot tính phí doanh nghiệp như thế nào

### Hai lớp chi phí cần phân biệt

```
Lớp 1: PayPilot trả phí cho Casso
       (1 App, tính theo TỔNG giao dịch của TẤT CẢ tenant cộng lại)
                     │
                     ▼
Lớp 2: PayPilot thu phí TỪNG doanh nghiệp
       (theo số giao dịch RIÊNG của doanh nghiệp đó/tháng)
```

Doanh nghiệp khách hàng **không** trả tiền trực tiếp cho Casso — họ chỉ liên kết ngân hàng qua Cas Link và trả phí dịch vụ cho PayPilot. PayPilot là bên đứng giữa, gộp chi phí hạ tầng (trả cho Casso) vào giá bán cho khách hàng cuối.
...
> **Định nghĩa nhất quán với Casso:** 1 giao dịch = 1 lần Cas Balance Hook bắn webhook về (1 lần biến động số dư), bất kể AI Matching kết quả là Auto Match, Human Review, hay Skip — vì PayPilot vẫn phải trả phí cho Casso ngay khi nhận webhook đó, không phụ thuộc kết quả xử lý sau này.

**Vì sao tính ngay lúc nhận webhook, không phải lúc AI xử lý xong?** Vì chi phí PayPilot trả cho Casso (lớp hạ tầng phía dưới) cũng tính theo số webhook nhận được, không phải theo việc AI ghép đúng hay sai. Dù giao dịch sau đó Auto Match, vào Human Review, hay bị Skip — cả 3 trường hợp đều đã tốn 1 lượt quota, vì backend đã phải nhận và xử lý webhook đó rồi.
...
- **Cas Partner** (xem mục dưới) là người duy nhất có quyền xem được tổng doanh thu PayPilot thu từ tất cả tenant, và doanh thu theo từng tenant cụ thể

### Nâng cấp gói qua PayOS — PayOS billing dành riêng cho PayPilot

> **Phân biệt quan trọng:** Đây là **PayOS account của chính PayPilot** (1 account duy nhất), khác hẳn với việc dùng PayOS để doanh nghiệp nhận tiền từ khách hàng cuối (đã quyết định bỏ, dùng QR ngân hàng công khai thay thế — xem README mục System Context). Ở đây, **PayPilot là bên bán**, doanh nghiệp là bên mua gói dịch vụ.
...
Bấm "Nâng cấp lên Pro — 799.000đ/tháng"
        │
        ▼
PayPilot Backend gọi PayOS API tạo Payment Link
body: {
  orderCode: "UPG-{tenant_id}-{timestamp}",
  amount: 799000,
  description: "PayPilot Pro - {tenant_id}",
  returnUrl, cancelUrl
}
        │
        ▼
...
POST /api/v1/webhook/payos-billing
        │
        ▼
PayPilot verify, đọc orderCode → tách ra tenant_id
        │
        ▼
...
**Vì sao `orderCode` chứa `tenant_id`:** Tương tự cách `grantId` định tuyến đúng tenant ở Cas Balance Hook, `orderCode` ở đây đóng vai trò định tuyến — vì PayOS callback chỉ trả về `orderCode` đã gửi đi, PayPilot cần tự encode `tenant_id` vào đó để biết doanh nghiệp nào vừa thanh toán xong.
...
GET    /api/v1/billing/invoices           # Lịch sử hóa đơn PayPilot xuất cho tenant này
...
| | Confirm ghép giao dịch | ✅ | ✅ | ❌ |
| | Reassign hóa đơn khác | ✅ | ✅ | ❌ |
| | Skip giao dịch | ✅ | ✅ | ❌ |
| **Invoices** | Xem danh sách / chi tiết hóa đơn | ✅ | ✅ | ✅ |
| | Tạo hóa đơn mới | ✅ | ✅ | ❌ |
| | Sửa hóa đơn | ✅ | ✅ | ❌ |
| | Xóa hóa đơn | ✅ | ✅ | ❌ |
| | Import hóa đơn từ Excel/CSV | ✅ | ✅ | ❌ |
| | Sinh QR ngân hàng công khai cho hóa đơn | ✅ | ✅ | ❌ |
| **Customers** | Xem danh sách / chi tiết khách hàng | ✅ | ✅ | ✅ |
| | Tạo khách hàng mới | ✅ | ✅ | ❌ |
...
| **Settings — AI Matching** | Xem ngưỡng confidence hiện tại | ✅ | ✅ | ❌ |
